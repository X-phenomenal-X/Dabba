    ...
    >
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .sheet-slide-up { animation: sheetSlideUp 0.3s cubic-bezier(.22,1,.36,1); }
        ...
        @keyframes sheetSlideUp { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } }
        .page-enter { animation: pageEnterRight 0.32s cubic-bezier(.22,1,.36,1); }
        .page-enter-right { animation: pageEnterRight 0.32s cubic-bezier(.22,1,.36,1); }
        .page-enter-left { animation: pageEnterLeft 0.32s cubic-bezier(.22,1,.36,1); }
        .nav-icon-bounce { animation: navBounce 0.4s cubic-bezier(.34,1.56,.64,1); }
        @keyframes navBounce { 0% { transform: scale(1); } 40% { transform: scale(1.28); } 100% { transform: scale(1); } }
        @keyframes pageEnterRight { from { opacity: 0; transform: translateX(16px);} to { opacity: 1; transform: translateX(0);} }
        @keyframes pageEnterLeft { from { opacity: 0; transform: translateX(-16px);} to { opacity: 1; transform: translateX(0);} }
        .check-pop { animation: checkPop 0.35s cubic-bezier(.34,1.56,.64,1); }
        .nav-btn { transition: color 0.2s ease, transform 0.15s ease; }
        .nav-btn:active { transform: scale(0.92); }
        .card-hover { transition: transform 0.18s ease, box-shadow 0.18s ease; box-shadow: 0 4px 14px rgba(0,0,0,0.28); transform-style: preserve-3d; perspective: 500px; }
        .card-hover:active { transform: perspective(500px) rotateX(3deg) scale(0.985); box-shadow: 0 1px 4px rgba(0,0,0,0.3); }
        .tiffin-tile-3d { transition: transform 0.15s ease; transform-style: preserve-3d; }
        .tiffin-tile-3d:active { transform: perspective(500px) rotateX(4deg) scale(0.98); }
        .elevated { box-shadow: 0 6px 22px rgba(0,0,0,0.35); }
        @keyframes dabbaSpin { from { transform: rotateY(0deg); } to { transform: rotateY(360deg); } }
      ...
      {/* Sticky header */}
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: "rgba(28,24,21,0.92)", backdropFilter: "blur(6px)", borderBottom: `1px solid ${PALETTE.border}`, paddingTop: "calc(env(safe-area-inset-top) + 6px)" }}>
      ...
      {/* Page content */}
      <div key={tab} className={`page-enter page-enter-${slideDirection}`} style={{ maxWidth: "480px", margin: "0 auto", position: "relative", zIndex: 1 }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        {tab === "today" && (
          <>
            {showWelcome && (
            ...
            {/* Tiffin stack */}
            <div style={{ padding: "1.25rem 1.25rem 0" }}>
              <div className="elevated" style={{ background: PALETTE.surface, borderRadius: "20px", padding: "0.6rem", border: `1px solid ${PALETTE.border}` }}>
                {TASKS.map((t, i) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      className={`tier-btn stagger-in tiffin-tile-3d${overdue ? " overdue-tile" : ""}`}
                    ...
                    >
                      ...
                      <div style={{ fontSize: "0.72rem", fontWeight: 600, color: done ? PALETTE.bg : PALETTE.textMutedBrown, whiteSpace: "nowrap" }}>{t.time}</div>
                    </button>
                  );
                })}
              </div>
      ...
      >
        <div style={{ maxWidth: "480px", margin: "0 auto", display: "flex", padding: "0.5rem 0.5rem calc(0.5rem + env(safe-area-inset-bottom))", position: "relative" }}>
          <div
            style={{
              position: "absolute",
              top: "0.5rem",
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <div style={{ position: "relative" }} className={active ? "nav-icon-bounce" : ""}>
